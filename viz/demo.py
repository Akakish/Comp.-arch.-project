...
def run_trace(cache, addrs):
    cache.flush()
    for addr in addrs:
        cache.access(addr)

def make_trace_sequential(n=2000, stride=64):
    return [i * stride for i in range(n)]

def make_trace_random(n=2000, addr_range=1 << 20):
    import random; random.seed(42)
    return [random.randint(0, addr_range) & ~63 for _ in range(n)]

def make_trace_thrash(n=2000, working_set=1 << 17):
    import random; random.seed(7)
    return [random.randint(0, working_set) & ~63 for _ in range(n)]

print("=" * 50)
print("  Cache Hierarchy Simulator — Visualization Demo")
print("=" * 50)

trace = make_trace_random()

print("[1/6] Hit rate vs cache size...")
sizes = [4*1024, 8*1024, 16*1024, 32*1024, 64*1024, 128*1024]
hit_rates_by_policy = {"LRU": [], "Clock": [], "RRIP": []}
...
print("[2/6] Hit rate vs associativity...")
assocs = [1, 2, 4, 8, 16]
hit_rates_assoc = {"LRU": [], "Clock": [], "RRIP": []}
for policy in ["LRU", "Clock", "RRIP"]:
    for a in assocs:
...
print("[4/6] 3C miss classification...")
try:
...
print("[6/6] Multi-level hierarchy...")
